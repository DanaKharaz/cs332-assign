...
trait NodeScala {
  ...
  def start(relativePath: String)(handler: Request => Response): Subscription = {
    ...
    val serverSubscription = Future.run() { cancellationToken =>
      def handleRequestsContinuously(): Future[Unit] = {
        if (cancellationToken.isCancelled) Future.successful(())
        else {
          listener.nextRequest().flatMap {
            case (request, exchange) => Future {
              respond(exchange, cancellationToken, handler(request))
            }.flatMap(_ => handleRequestsContinuously())
          }
        }
      }

      handleRequestsContinuously()
    }
